...
> Hermes 部署测试请先看 [Hermes MediaCrawler MCP 部署测试指南](Hermes_MediaCrawler_MCP_部署测试指南.md)。默认主链路是 Windows 桌面真实浏览器采集后导出 dataset bundle，Hermes MCP 只负责数据集登记、标准化、查询和报告生成。
