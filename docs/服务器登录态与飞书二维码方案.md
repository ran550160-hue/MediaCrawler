...
> 2026-07 状态说明：本文保留为服务器采集实验/兜底方案。小红书生产主链路已调整为桌面端真实浏览器采集，服务器 MCP 负责数据集登记、查询、分析和报告。本文涉及的 QR、login、cookie、server collection 工具默认不注册给 Hermes，只有显式启用 `experimental_collection` profile 时才可见。详见 [桌面采集与数据集 MCP 架构改造方案](桌面采集与数据集MCP架构改造方案.md)。
...
以下是历史实验流程，不是默认 `dataset` profile 流程：

1. 用户显式要求测试服务器采集。
2. 管理员显式启用 `experimental_collection` profile。
3. Hermes 调用 MediaCrawler Research MCP 创建采集任务。
4. MCP 发现小红书未登录，返回 `need_login`。
5. Hermes 调用 MCP 获取登录二维码。
6. Hermes 将二维码图片通过飞书机器人发给用户。
7. 用户用手机扫码登录。
8. MCP 轮询或接收登录完成状态。
9. Hermes 继续执行原采集任务。
...
sequenceDiagram
    ...
    U->>H: 采集小红书关键词数据
    H->>M: start_collection(dataset_id, experimental_collection enabled)
    ...
    M->>B: 轮询登录状态
    M-->>H: login_success
    H->>M: retry start_collection(...)
    M-->>H: dataset_id + report paths
    H-->>U: 返回采集与分析结果
```

默认主流程请使用“桌面采集数据 -> 导入数据集 -> normalize -> query -> generate_report”，不要使用上图流程。
